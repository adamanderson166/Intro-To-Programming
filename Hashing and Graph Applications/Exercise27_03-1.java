...
Adam Anderson
4/9/2019
(Implement MyMap using open addressing with double hashing) Create a new concrete class that implements MyMap using open addressing with double hashing. For simplicity, use f(key) = key % size as the hash function, where size is the hash-table size. The second function should be g(key) = prime- key % prime, where prime is a prime number of your choosing. Initially, the hash-table size is 4. The table size is doubled whenever the load factor exceeds the threshold (0.5) (this should be done for you already).

After incrementing i, you need to mod by table.length so that your keys wrap around the array indices.

Here is the code for Exercise 27.2 which implements MyMap using quadratic probing. Modify this to use double hashing for open addressing. Edit get, put, and remove.
**/
